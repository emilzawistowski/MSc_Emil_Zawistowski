...
def preprocess_all_vmrk_files(
    ...
    """
    # Create output folder using pathlib
    ...
    for input_file_path in files:
        # Build output path using pathlib
        output_file_path = output_folder / input_file_path.name

        with open(input_file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()

        # Identify block type based on unique stimulus codes
        ...
        # Split file content into header and marker segments
        for line in lines:
        ...
        for line in marker_lines:
            ...
            # 1. Filter out response markers
            if m_type.lower() == "response":
                continue

            # 2. Re-label first 'New Segment' as 'block start'
            # Changes the first marker in the block to S 99, according to TRIGGER_CODES in config.py
            ...
            # 3. Standardize stimulus codes
            # Map S 5/S 6/S 15 to S 21/S 22/S 31 to match the dictionary in config.py
